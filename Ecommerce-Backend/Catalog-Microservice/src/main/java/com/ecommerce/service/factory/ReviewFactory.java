...
public class ReviewFactory {
    ...
    public Review create(ReviewRequest request) {
        ...
        log.debug(
                "Creating review entity for user ID: {}",
                request.getUserId()
        );
        ...
        Long reviewId =
                mongoSequenceService.getNextSequence("reviews");

        review.setReviewId(reviewId);

        log.debug(
                "Review entity created successfully with ID: {}",
                reviewId
        );
    ...
    public void validateDuplicateReview(
            Product product,
            Long userId) {

        log.debug(
                "Validating duplicate review for product ID: {} and user ID: {}",
        ...
        if (reviewRepository.existsByProductIdAndUserId(
            ...
            log.warn(
                    "Duplicate review detected for product ID: {} and user ID: {}",
                    product.getProductId(),
                    userId
            );
